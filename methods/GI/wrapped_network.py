...
class TimeReLUCompatible(nn.Module):
    ...
    def __init__(self, time_shape, leaky=False, use_time=True, deep=False, name="timerelu"):
    ...
    def forward(self, X):
        ...
        # print(self.name, self.times is None)
        assert self.times is not None, f"[{self.name}] Error: please call self.set_times(times) for this forward process"
...
class WrappedGIFeature(nn.Module):
    ...
    def replace_relu_with_timerelu(network: nn.Module, time_dim, num_replace=None):
        ...
        for name in timerelu_names:
            ...
            # print("adding TReLU to,", name, parent_name, relu_name)
            parent_module = network.get_submodule(parent_name)
            time_relu = TimeReLUCompatible(time_dim, leaky=True, use_time=True,name = name).cuda()
    ...
    def forward(self, x, time_v):
        ...
        # self.set_times_for_all_timerelu(time_v)
        # pdb.set_trace()
        out = self.feature(x)
        # pdb.set_trace()
        if self.first_forward:
    ...
    def set_time_for_timerelu(self, time):
        for name in self.timerelu_names:
            ...
            # print("setting time for timerelu: ", name)
    
    def clean_time_for_timerelu(self):
        for name in self.timerelu_names:
            timerelu: TimeReLUCompatible = self.feature.get_submodule(name)
            timerelu.set_times(None)
            # print("cleaning time for timerelu: ", name)
    
    def set_times_for_all_timerelu(self, times: torch.Tensor):
        for module in self.modules():
            if isinstance(module, TimeReLUCompatible):
                module.set_times(times)
