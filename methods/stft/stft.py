...
class STFT(BaseTrainer):
    ...
    def __str__(self):
        return f'STFT-K={self.K}-{self.base_trainer_str}'
    ...
    def train_step(self, dataloader, cl_flag=False, split_epoch = 1, ts = 0):
        ...
        if cl_flag:
            freeze_normalization_layers(self.network)
            base_path = Path(self.args.base_dir) / f"{self.args.method}_{self.args.dataset}_{self.epochs}_{self.args.lr}" / f"{self.args.mini_batch_size}_{split_epoch}_{self.args.split_lr}"/ f"{ts}"
            ...
            stop_iters = split_epoch * (max(self.split_set.__len__() // self.args.mini_batch_size, 1)) - 1
        else:
            base_path = Path(self.args.base_dir) / f"{self.args.method}_{self.args.dataset}_{self.epochs}_{self.args.lr}" / f"{self.args.mini_batch_size}_{split_epoch}_{self.args.split_lr}"/ "base"
            base_path.mkdir(parents=True, exist_ok=True)
            ...
            stop_iters = self.args.epochs * (max(self.train_dataset.__len__() // self.args.mini_batch_size, 1)) - 1
        ...
        for step, (x, y) in enumerate(dataloader):
            ...
            if step % (max(1, stop_iters // 5)) == 0:
                ...
                if cl_flag:
                    ...
                    param_path = base_path / f"step_{step}.pth"
                    torch.save(cur_params, param_path)
                    split_table.to_csv(Path(self.args.base_dir) / f"{self.args.method}_{self.args.dataset}_{self.epochs}_{self.args.lr}" / f"{self.args.mini_batch_size}_{split_epoch}_{self.args.split_lr}"/f"{ts}.csv")

        self.logger.info("-------------------end training on timestamp {}-------------------".format(self.train_dataset.current_time))
        
    # def train_online_old(self):
    #     self.train_dataset.mode = 0
    #     for i, timestamp in enumerate(self.train_dataset.ENV[:-1]):
    #         if self.args.eval_fix and timestamp == (self.split_time + 1):
    #             break
    ...
    # def train_step_old(self, dataloader, freeze_norm=False, rw_configs=None, user_iters = -1, user_lr_decay = False,ts = -1, idd = -1,cl_flag = False):
    #     self.logger.info("-------------------start training on timestamp {}-------------------".format(self.train_dataset.current_time))
    #     self.network.train()
    #     # if freeze_norm:
    #     #     freeze_normalization_layers(self.network)
    ...
    #     if cl_flag:
    #         freeze_normalization_layers(self.network)
    #         base_path = Path(self.args.base_dir) / f"{self.args.method}_{self.args.dataset}_{self.epochs}" / f"{self.args.mini_batch_size}_{self.args.split_epochs}_{self.args.split_lr}"/ f"{ts}"
    #         base_path.mkdir(parents=True, exist_ok=True)
    #         split_table = pd.DataFrame()
    #         self.logger.info("-------------------start training on timestamp {}-------------------".format(self.split_set.current_time))
    #         self.logger.info("self.split_set.len = {} x {} = {} samples".format(self.split_set.__len__() // self.args.mini_batch_size, self.args.mini_batch_size, self.split_set.__len__()))
    #         stop_iters = self.args.epochs * (self.split_set.__len__() // self.args.mini_batch_size) - 1
    #     # base_path = Path(self.args.base_dir) / f"{self.args.method}_{self.args.dataset}_{self.epochs}_{self.args.freeze_start}" / f"{self.args.mini_batch_size}_stsi_{self.args.lr}_{self.args.freeze_lr_ratio}"/ f"{ts}"
    #     base_path.mkdir(parents=True, exist_ok=True)
    #     split_table = pd.DataFrame()
    #     meters = MetricLogger(delimiter="  ")
    #     end = time.time()
    ...
    #             param_path = base_path / f"step_{step}.pth"
    #             torch.save(cur_params, param_path)
    #             split_table.to_csv(Path(self.args.base_dir) / f"{self.args.method}_{self.args.dataset}_{self.epochs}_{self.args.freeze_start}" / f"{self.args.mini_batch_size}_stsi_{self.args.lr}_{self.args.freeze_lr_ratio}"/f"{ts}.csv")
