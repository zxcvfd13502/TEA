...
from .resnet_gi import resnet50_gi, resnet18_gi
...
class Clear100Network(nn.Module):
    def __init__(self, args, weights=None, ssl_training=False):
        super(Clear100Network, self).__init__()
        self.args = args
        self.num_classes = NUM_CLASSES
        if args.use_r50:
            print("Using the resnet 50 model now")
            self.enc = torchvision.models.resnet50(pretrained=False)
            if args.method == 'gi':
                print("Using the resnet 50 model for gi now")
                self.enc = resnet50_gi()
            del self.enc.fc
            self.enc.fc = Identity()
            # self.enc = nn.Sequential(densenet121(pretrained=False).features, nn.ReLU(), nn.AdaptiveAvgPool2d(output_size=(1, 1)), nn.Flatten())
            self.feature_dim = 2048
        else:
            print("Using the resnet 18 model now")
            self.enc = torchvision.models.resnet18(pretrained=False)
            if args.method == 'gi':
                print("Using the resnet 18 model for gi now")
                self.enc = resnet18_gi()
            del self.enc.fc
            self.enc.fc = Identity()
            # self.enc = nn.Sequential(densenet121(pretrained=False).features, nn.ReLU(), nn.AdaptiveAvgPool2d(output_size=(1, 1)), nn.Flatten())
        ...
        if args.use_densenet:
            print("using densenet for clear100")
            if args.dim_bottleneck_f is not None:
                self.bottleneck = nn.Sequential(
                    nn.Linear(1024, args.dim_bottleneck_f),
                    nn.BatchNorm1d(args.dim_bottleneck_f),
                    nn.ReLU()
                )
                self.enc = nn.Sequential(densenet121(pretrained=False).features, nn.ReLU(), nn.AdaptiveAvgPool2d(output_size=(1, 1)), nn.Flatten(), self.bottleneck)
                self.feature_dim = args.dim_bottleneck_f
            else:
                self.enc = nn.Sequential(densenet121(pretrained=False).features, nn.ReLU(), nn.AdaptiveAvgPool2d(output_size=(1, 1)), nn.Flatten())
...
class Clear100Network_for_EvoS(nn.Module):
    def __init__(self, args, weights=None):
        super(Clear100Network_for_EvoS, self).__init__()
        self.args = args
        self.num_classes = NUM_CLASSES
